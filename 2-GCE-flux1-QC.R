...
#   b. EddyPro QC falgs
#   c. remove negative nighttime fluxes
#   d. remove extreme values
#   e. u* filtering

#Output:
# 1. flux dataset ready for combination with flux2
...
#### no negative nighttime ============================================================================================

full_data_qc_removal_night <- full_data_qc_removal %>%
  mutate(day_night = as.factor(ifelse(Solar_elv > -5, 'day','night'))) %>%
  mutate(NEE_maint_qc_removal_night = ifelse(day_night == 'night' & NEE_maint_qc_removal < 0, NA, NEE_maint_qc_removal)) %>%
  mutate(LE_maint_qc_removal_night = ifelse(day_night == 'night' & LE_maint_qc_removal < 0, NA, LE_maint_qc_removal)) %>%
  mutate(H_maint_qc_removal_night = ifelse(day_night == 'night' & H_maint_qc_removal < 0, NA, H_maint_qc_removal))

# full_data_qc_removal_night %>%
#   ggplot(aes(x=datetime))+
#   geom_point(aes(y=NEE_maint_qc_removal),color='red',size=0.25)+
#   geom_point(aes(y=NEE_maint_qc_removal_night),color='black',size=0.25)+
...
full_data_qc_removal_night_ext <- full_data_qc_removal_night %>%
  mutate(NEE_maint_qc_removal_night_ext = ifelse(between(NEE_maint_qc_removal_night,-20,15),NEE_maint_qc_removal_night,NA),
         LE_maint_qc_removal_night_ext = ifelse(between(NEE_maint_qc_removal_night,-20,15),LE_maint_qc_removal_night,NA),
         H_maint_qc_removal_night_ext = ifelse(between(NEE_maint_qc_removal_night,-20,15),H_maint_qc_removal_night,NA))

# full_data_qc_removal_night_ext %>%
#   ggplot(aes(x=datetime))+
#   geom_point(aes(y=NEE_maint_qc_removal_night),color='red',size=0.25)+
#   geom_point(aes(y=NEE_maint_qc_removal_night_ext),color='black',size=0.25)+
...
fluxes_reddy <- full_data_qc_removal_night_ext %>%
  mutate(Mean_Shortwave_Net = Mean_PAR_Incident_Tower/2) %>%
  select(DateTime=datetime,NEE=NEE_maint_qc_removal_night_ext,Rg=Mean_Shortwave_Net,Tair=Mean_Temp_Air,VPD,Ustar=`u*`) %>%
  as.data.frame()


#convert to whatever object REddyProc likes
fluxes_reddyproc <- sEddyProc$new('gce',fluxes_reddy,c('NEE','Rg','Tair','VPD','Ustar'))

#ustar threshold determination with bootstrapping
# ustarseasonal <- fluxes_reddyproc$sEstimateUstarScenarios(
#   nSample = 100L, probs = c(0.05, 0.5, 0.95))
ustarthres <- fluxes_reddyproc$sEstUstarThold()
# outDir <- getwd()
# fluxes_reddyproc$sPlotNEEVersusUStarForSeason(levels(ustarthres$season)[3], dir = outDir )

ustar_seasonal_values <- ustarthres %>%
  as_tibble() %>%
  drop_na() %>%
  ...
  mutate(end_date = if_else(is.na(end_date),as.Date('2017-12-31'),end_date)) %>% # for flux 1
  select(start_date,end_date,uStar)


#remove data if less than u* threshold
full_data_qc_removal_night_ext_ustar <- full_data_qc_removal_night_ext %>%
  mutate(date = as.Date(datetime)) %>%
  fuzzy_left_join(ustar_seasonal_values,
                  by=c('date'='start_date','date'='end_date'),
                  match_fun=list(`>=`,`<`)) %>%
  mutate(NEE_maint_qc_removal_night_ext_ustar = ifelse(`u*`<uStar,NA,NEE_maint_qc_removal_night_ext),
         LE_maint_qc_removal_night_ext_ustar = ifelse(`u*`<uStar,NA,LE_maint_qc_removal_night_ext),
         H_maint_qc_removal_night_ext_ustar = ifelse(`u*`<uStar,NA,H_maint_qc_removal_night_ext)) 


# full_data_qc_removal_night_ext_ustar %>%
#   ggplot(aes(x=datetime))+
#   geom_point(aes(y=NEE_maint_qc_removal_night_ext),color='red')+
#   geom_point(aes(y=NEE_maint_qc_removal_night_ext_ustar),color='black')+
#   scale_x_datetime(date_breaks='1 year',date_labels = '%Y')
...
#check column names
colnames(full_data_qc_removal_night_ext_ustar)

#rename NEE to org and remove unwanted columns
out <- full_data_qc_removal_night_ext_ustar %>%
  rename(NEE_org = NEE,
...
out %>%
  pivot_longer(cols=c(contains('_ustar'))) %>%
...
write.csv(out,paste0('processed/b_',
